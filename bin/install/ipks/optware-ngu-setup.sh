...
echo -e $INFO Script modifies only \"optware-ng\" folder on the chosen drive,
...
case $(uname -m) in
  armv7l)
    ...
    INST_URL='http://ipkg.nslu2-linux.org/optware-ng/bootstrap/buildroot-armeabi-ng-bootstrap.sh'
    OPT_FOLD='optware-ng.arm'
    ;;
  mips)
    PART_TYPES='ext2|ext3'
    INST_URL='http://ipkg.nslu2-linux.org/optware-ng/bootstrap/buildroot-mipsel-ng-bootstrap.sh'
    OPT_FOLD='optware-ng'
    ;;
  *)
...
if [ -d "$entwarengFolder" ] ; then
  ...
  mv "$entwarengFolder" "$entwarengFolder-old_$(date +%F_%H-%M)"
fi

if [ -d "$entwarengarmFolder" ] ; then
  echo -e "$WARNING Found Entware-NG Arm installation, saving..."
  mv "$entwarengarmFolder" "$entwarengarmFolder-old_$(date +%F_%H-%M)"
fi
...
echo -e "$INFO If the script stops here with no activity for more than a minute,"
echo -e "$INFO optware-ng feeds url is down, try again later..."
...
echo -e "$INFO http://ipkg.nslu2-linux.org/optware-ng/buildroot-mipsel-ng/Packages.html"
echo -e "$INFO List of installable arm packages"
echo -e "$INFO http://ipkg.nslu2-linux.org/optware-ng/buildroot-armeabi-ng/Packages.html"
